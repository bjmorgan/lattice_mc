...
class TranstitionsTestCase(unittest.TestCase):
    ...
    def test_transitions_is_initialised(self):
        ...
        np.testing.assert_array_equal(
            self.transitions.p, np.array([0.25, 0.25, 0.25, 0.25])
        )

    def test_cumulative_probabilities(self):
        self.transitions.p = np.array([0.1, 0.2, 0.3, 0.4])
        np.testing.assert_allclose(
            self.transitions.cumulative_probabilities(), np.array([0.1, 0.3, 0.6, 1.0])
        )

    @patch("random.random")
    def test_random(self, mock_random):
        self.cumulative_probabilities = Mock(
            return_value=np.array([0.1, 0.3, 0.6, 1.0])
        )
